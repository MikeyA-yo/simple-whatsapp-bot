 ...
 * @returns {boolean} Whether the string is alphabetical or not.
 */
const isAlphabetical = (str) => /[a-z]/i.test(str);
 ...
 * @param {string} concealChar - The conceal character to swap the word's letter with.
 * @returns {array} The conceal word array.
 */
const createConcealArr = (word, concealChar = '_') => {
  return [...toConsumableArray(word)].map((letter) =>
    isAlphabetical(letter) ? concealChar : letter
  );
};
 ...
 * @returns {array} - Array of the item's occurrences in the provided array.
 */
const getAllIndexes = (array, itemToFind) => {
 ...
 */
const changeArrayItems = (arr, arrayToSwap, indexes) => {
  const newArr = [...toConsumableArray(arr)];
...
function Game(word, config) {
  ...
  this.charactersMap = [...toConsumableArray(word)];
  this.uppercaseMap = [...toConsumableArray(word)].map((c) => c.toUpperCase());
  this.guessedLetters = [];
  this.failedLetters = [];
...
Game.prototype.guess = function (char) {
  ...
  const guessedLetters = [...toConsumableArray(this.guessedLetters)];
  if (!guessedLetters.includes(char)) {
    this.guessedLetters = [...toConsumableArray(this.guessedLetters), char];
  ...
  if (!this.hiddenWord.includes(this.config.concealCharacter)) {
    this.status = 'WON';
  }
 ...
 */
Game.prototype.revealHiddenWord = function () {
